...
if(isset($_POST['submit'])!=""){
  ...
  $chk = $conn->query("SELECT * FROM  tb_reporte_avance where name = '$name' ")->rowCount();
  if($chk){
    $i = 1;
    $c = 0;
	while($c == 0){
    ...
    // var_dump($tname);exit;
    	$chk2 = $conn->query("SELECT * FROM  tb_reporte_avance where name = '$tname' ")->rowCount();
    	if($chk2 == 0){
    		$c = 1;
    		$name = $tname;
    	}
 ...
 if($move){
 	$query=$conn->query("insert into tb_reporte_avance(name,fname)values('$name','$fname')");
	if($query){
	header("location:subirAvance");
	}
	else{
	die(mysql_error());
	}
...
?>
    ...
    <div class="span12">
        <div class="container">
            <br />
            <h1>
                <p>Subir FORMAC-30 Reporte Avance del proyecto</p>
            </h1>
            <br />
            <br />
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                ...
                <?php
			$query=$conn->query("select * from tb_reporte_avance order by id desc");
